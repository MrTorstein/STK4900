...
# Question a)
# Our starting point in this exercise is the model for CHD risk using the predictors age (per 10 years), cholesterol (per 50 mg/dL), 
# systolic blood pressure (per 50 mmHg), body mass index (per 10 kg/m2), smoking (yes, no),
...
# All the different parameters seem to have som form of significanse, except for A2. B4 is not very significant, just a little.
print("--------------------------------------------------------------------------")
...
# We get a diviance of 0.23 and a 0 value of 0.89. This means we can not discard the null hypotesis, and it is likely that the As are equal and the Bs are too.
print("---------------------------------------------------------------------------")
...
# Here we get the diviance of 24, with a p-value of 7 * 10^-7 for the binary pattern. But a 0.2 diviance and 0.89 p-value for the categorical behavior.
# From this we can conclude that we need the binary pattern, but we can not conclude about the categorical behavior.
print("---------------------------------------------------------------------------")

# Question d)
# Which of the models in question a, b, and c would you prefer?

# I perfere the model in b)
...
# All the different factors raise the chance of getting CDH.
# This means the riskfactor for CHS of a person, 10 years older, with 50 mmHg higher blood pressure, 50 mg/dL higher cholesterol, and 10 kg/m^2 higher bmi and smokes,
# than another person has a 1.83025048 + 1.70240514 + 2.46791666 + 1.73236105 + 1.82916180 + 2.00685487 = 11.5 higher chance.